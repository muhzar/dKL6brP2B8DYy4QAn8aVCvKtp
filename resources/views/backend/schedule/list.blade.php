              ...
              <table class="table table-hover">
                ...
                @foreach($posts as $post)
                  <tr>
                    ...
                    <td>{{ date('d F Y', strtotime($post->assign_date)) }}</td>
                    <td>{{ $post->getTrack->getCluster->name }}</td>
                    <td>{{ $post->getShift->name }}</td>
                    <td>{{ formatedDate($post->created_at) }}</td>
                    <td width="100"><form id="form-delete" method="POST" action="{{ route('schedule.destroy', $post->id)  }}">{{ csrf_field() }} {{ method_field('DELETE') }}<button type="button" class="btn-delete btn btn-sm btn-block btn-danger" style="width: 80px">Delete</button></form></td>
                    <td width="100"><form id="form-update" method="GET" action="{{ route('schedule.edit', $post->id)  }}"><button type="submit" class="btn btn-sm btn-block btn-warning" style="width: 80px">Edit</button></form></td>
                  </tr>
                @endforeach
              </tbody>
              </table>
