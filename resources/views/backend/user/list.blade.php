              ...
              <table class="table table-hover">
                ...
                @foreach($users as $user)
                  <tr>
                    ...
                    <td>{{ getUsername($user->created_by) }}
                    <td>{{ formatedDate($user->created_at) }}
                    <td width="100"><form class="form-delete" method="POST" action="{{ route('user.destroy', $user->id)  }}">{{ csrf_field() }} {{ method_field('DELETE') }}<button type="button" class="btn-delete btn btn-sm btn-block btn-danger" style="width: 80px">Delete</button></form></td>
                    <td width="100"><form class="form-update" method="GET" action="{{ route('user.edit', $user->id)  }}"><button type="submit" class="btn btn-sm btn-block btn-warning" style="width: 80px">Edit</button></form></td>
                  </tr>
                @endforeach
              </tbody>
              </table>
