              ...
              <table class="table table-hover">
                ...
                @foreach($posts as $post)
                  <tr>
                    <td>{{ $i++ }}</td>
                    <td>{{ $post->title }}</td>
                    <td>{{ $post->category->getcategory->title }}</td>
                    <td>{{ status($post->status) }}</td>
                    <td>{{ getUsername($post->created_by) }}</td>
                    <td>{{ formatedDate($post->created_at) }}</td>
                    <td>{{ formatedDate($post->updated_at) }}</td>
                    <td width="100"><form id="form-delete" method="POST" action="{{ route('page.destroy', $post->id)  }}">{{ csrf_field() }} {{ method_field('DELETE') }}<button type="button" class="btn-delete btn btn-sm btn-block btn-danger" style="width: 80px">Delete</button></form></td>
                    <td width="100"><form id="form-update" method="GET" action="{{ route('page.edit', $post->id)  }}"><button type="submit" class="btn btn-sm btn-block btn-warning" style="width: 80px">Edit</button></form></td>
                  </tr>
                @endforeach
              </tbody>
              </table>
