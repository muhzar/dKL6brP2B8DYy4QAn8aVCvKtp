        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><small><a class="btn btn-default" href="{{ url('cms/track/create') }}">add new</a></small></h3>
              ...
              <table class="table table-hover">
                ...
                @foreach($users as $user)
                  <tr>
                    ...
                    <td>{{ $user->code }}</td>
                    <td>{{ $user->name }}</td>
                    <td>{{ $user->cluster_code }}</td>
                    <td>{{ formatedDate($user->created_at) }}
                    <td width="100">
                        <form class="form-delete" method="POST" action="{{ route('track.destroy', $user->id)  }}">{{ csrf_field() }} {{ method_field('DELETE') }}
                            <button type="button" class="btn-delete btn btn-sm btn-block btn-danger" style="width: 80px">Delete</button>
                        </form>
                    </td>
                    <td width="100">
                        <form class="form-update" method="GET" action="{{ route('track.edit', $user->id)  }}">
                            <button type="submit" class="btn btn-sm btn-block btn-warning" style="width: 80px">Edit</button>
                        </form>
                    </td>
                    <!-- <td>set point</td>
                    <td>set track</td> -->
